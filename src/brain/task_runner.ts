...
export class TaskRunner {
  private taskRegistry: Record<AllowedTask, TaskFunction>;
  ...
  public async executeTask(incomingPacket: IntentPacket): Promise<IntentPacket> {
    const details = incomingPacket.payload.details as any;
    const task_name = details?.task_name as AllowedTask;
    const parameters = details?.parameters;

    if (!task_name || !this.taskRegistry[task_name]) {
      return {
    ...
    try {
      const taskFunction = this.taskRegistry[task_name];
