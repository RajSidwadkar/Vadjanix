...
/**
 * Enforces a timeout on a promise.
 */
export async function withTimeout(promise: Promise<IntentPacket>, ms: number = 10000): Promise<IntentPacket | null> {
  let timeoutId: NodeJS.Timeout;
  const timeoutPromise = new Promise<null>((resolve) => {
    timeoutId = setTimeout(() => {
      console.warn(`[SWARM WARNING] Sub-agent timed out after ${ms}ms`);
      resolve(null);
    }, ms);
  });
...
/**
 * Aggregates results from multiple sub-agents based on the specified strategy.
 */
export async function aggregateResults(results: (IntentPacket | null)[], strategy: AggregationStrategy): Promise<IntentPacket> {
