...
import { HybridLLMProvider } from './HybridLLMProvider.js';
...
export async function createAdapter(config: { provider: string }, vault?: SecureVault): Promise<ILLMProvider> {
  ...
  const cloud_llm = new GeminiAdapter(vault);
  ...
  if (config.provider === 'hybrid') {
    return new HybridLLMProvider(slm, cloud_llm);
  }
