...
async function main() {
    ...
    await runTest('Test 1: Embedding & Episodic Storage', async () => {
        ...
        const row = db.prepare('SELECT * FROM episodic WHERE raw_exchange = ?').get('User asked about the weather');
        assert.ok(row);
    ...
    await runTest('Test 3: Causal Graph Integrity', async () => {
        ...
        const db = new Database(testDbPath);
        const row = db.prepare('SELECT * FROM causal_graph WHERE cause = ?').get('Heavy rain');
        assert.ok(row);
