...
async function runTests() {
  ...
  test("Strategy: walk_away", async () => {
    const packet = await evaluateProposal('walk_away', 'buyer', 150, 100, 10);
  ...
  test("Strategy: hold_firm", async () => {
    const packet = await evaluateProposal('hold_firm', 'seller', 160, 200, 10);
  ...
  test("Strategy: compromise (Buyer)", async () => {
    const packet = await evaluateProposal('compromise', 'buyer', 150, 100, 10);
    assert.strictEqual(packet.action, 'propose');
    assert.ok(packet.payload.message.includes("$110"), "Buyer should move from 100 to 110");
  });

  // --- Test 4: Seller Compromise (Price Goes Down) ---
  test("Strategy: compromise (Seller)", async () => {
    const packet = await evaluateProposal('compromise', 'seller', 160, 200, 15);
    assert.strictEqual(packet.action, 'propose');
    assert.ok(packet.payload.message.includes("$185"), "Seller should move from 200 to 185");
  });

  // --- Test 5: Reaching the Limit (Buyer) ---
  test("Reaching Limit (Buyer)", async () => {
    // Last offer 145, limit 150, step 10 -> Should be capped at 150
    const packet = await evaluateProposal('compromise', 'buyer', 150, 145, 10);
  ...
  test("Reaching Limit (Seller)", async () => {
    // Last offer 165, limit 160, step 10 -> Should be capped at 160
    const packet = await evaluateProposal('compromise', 'seller', 160, 165, 10);
  ...
  test("Already at Limit (Hold Firm)", async () => {
    const packet = await evaluateProposal('compromise', 'buyer', 150, 150, 10);
    assert.ok(packet.payload.message.includes("$150"));
    assert.strictEqual(packet.payload.details?.strategy, 'hold_firm');
  });
